      ...
      <div className="language-container">
        ...
        <Grid container spacing={4} justifyContent="center">
          {skills.map((skill, index) => (
            <Grid item xs={12} sm={6} key={index}> {/* 1 élément par ligne pour petits écrans, 2 pour moyens */}
              <motion.div
              ...
              >
                <Box className="skill" sx={{ textAlign: 'center' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', mt: 2, justifyContent: 'center' }}>
                    <skill.Icon size="8rem" className="skill-icon" />
                    <Box sx={{ width: '28vw', ml: 2, position: 'relative', display: 'flex', alignItems: 'center' }}>
                      ...
                      <Typography variant="body2" color="textSecondary">
                        <span className="skill-percentage" style={{ fontSize: '1vw' }}>{`${skill.percentage}%`}</span>
                      </Typography>
                    </Box>
                  </Box>
                </Box>
